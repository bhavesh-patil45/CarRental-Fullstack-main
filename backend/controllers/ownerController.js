...
export const addCar = async (req, res)=>{
    try {
        ...
        const cityName = car.location || "New York";
        ...
        await Car.create({...car, location: formattedLocation, owner: _id, image})
...
export const getDashboardData = async (req, res) =>{
    try {
        ...
        // Calculate monthlyRevenue from bookings where status is confirmed and pickup month is current month (Option B)
        const currentMonth = new Date().getMonth();
        const currentYear = new Date().getFullYear();
        const monthlyRevenue = bookings.slice().filter(booking => {
            const pickup = new Date(booking.pickupDate);
            return booking.status === 'confirmed' && 
                   pickup.getMonth() === currentMonth && 
                   pickup.getFullYear() === currentYear;
